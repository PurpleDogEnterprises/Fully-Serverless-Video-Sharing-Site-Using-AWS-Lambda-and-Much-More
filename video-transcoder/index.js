...
const handler = (event, context, callback) => {
    ...
    // Get the pipeline ID from the environment variables and the key name from the event passed in
    const pipelineId = process.env.ELASTIC_TRANSCODER_PIPELINE_ID;
    const key = event.Records[0].s3.object.key;

    // Input file may have spaces so replace them with '+'
    ...
    // Call function that creates an ElasticTranscoder Job
